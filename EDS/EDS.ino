...
short min_limit[PADS]    = {100,  100,  100,  100,  100,  100,  300,  300,  300};    // Нижний порог срабатывания (больше значение - больше усилий при уддаре)                
short scan_time[PADS]    = {20,   20,   20,   20,   20,   20,   20,   20,   20 };    // Частота опроса / (длительность игнорирования) датчиков (мс) 
short sensitivity[PADS]  = {80,   80,   80,   80,   80,   80,   80,   80,   80 };    // Чувствительность (больше значение - тише звучание)

boolean playing[PADS]    = {false,false,false,false,false,false,false,false,false};  // проигрывание ноты в данный момент
short high_score[PADS]   = {0,    0,    0,    0,    0,    0,    0,    0,    0};      // громкость (меняется в playNote)
unsigned long timer[PADS]= {0,    0,    0,    0,    0,    0,    0,    0,    0};      // время последнего срабатывания (мс)
...
void loop() {
  ...
  for (byte i = 0; i < PADS; i++) {    
    ...
    // значение датчика в пределе срабатывания и нота не проигрывается
    if (volume >= min_limit[i] && playing[i] == false) {
      // и датчик доступен для опроса
    ...
    // значение датчика в пределе срабатывания и нота проигрывается
    else if (volume >= min_limit[i] && playing[i] == true) {
      playNote(i, volume);
    }

    // значение датчика НЕ в пределе срабатывания и нота проигрывается
    else if (volume < min_limit[i] && playing[i] == true) {
      ...
      high_score[i] = 0;    // обнуление громкости
      playing[i] = false;   // отмечаем ноту как "не проигрывается"
      timer[i] = millis();  // запоминаем последнее время срабатывания
    }
  }
